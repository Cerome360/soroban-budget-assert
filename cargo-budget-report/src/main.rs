...
struct BudgetReportArgs {
    ...
    /// Restrict which cdylib packages are built and simulated.
    /// Can be repeated to select multiple packages.
    #[arg(long)]
    package: Vec<String>,

    /// Restrict which exported functions are simulated.
    /// Can be repeated to select multiple functions.
    #[arg(long)]
    function: Vec<String>,

    /// Bypass the contract ID cache and force a fresh deployment.
    #[arg(long, default_value_t = false)]
    force_deploy: bool,
}
