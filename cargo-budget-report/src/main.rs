...
/// Protocol-level maximum resource limits for Stellar Protocol 22.
///
/// These are the ceiling values enforced by the Soroban host for a single
/// transaction. Percentage-based budget limits (`*_limit_pct`) are resolved
/// against these constants so that `cpu_limit_pct = 90` means "90 % of the
/// protocol's max CPU instructions" rather than a hard-coded absolute number.
const PROTOCOL_MAX_CPU_INSTRUCTIONS: u64 = 10_000_000;
const PROTOCOL_MAX_READ_BYTES: u64 = 1_048_576;
const PROTOCOL_MAX_WRITE_BYTES: u64 = 1_048_576;
...
struct FunctionConfig {
    ...
    /// Percentage of the protocol max CPU instructions (10 000 000). Mutually
    /// exclusive with `cpu_limit`. `None` means the metric is reported but not
    /// enforced by `--check`.
    #[serde(default)]
...
fn limit_for_metric(func_config: &FunctionConfig, metric: &str) -> Option<u64> {
    match metric {
        "CPU Instructions" => func_config.cpu_limit.or(func_config.cpu_limit_pct),
        "Read Bytes" => func_config.read_limit.or(func_config.read_limit_pct),
        "Write Bytes" => func_config.write_limit.or(func_config.write_limit_pct),
        _ => None,
    }
}

/// Resolves the effective absolute limit for a metric from a `FunctionConfig`.
///
/// If an absolute `*_limit` is set it is returned directly. If a
/// `*_limit_pct` is set it is converted to an absolute value by multiplying
/// the percentage against the protocol max for that metric. If both are set
/// for the same metric, the absolute limit takes precedence and the
/// percentage is silently ignored.
///
/// Returns `None` when neither form is configured (metric is reported but
/// not enforced).
fn resolve_limit(func_config: &FunctionConfig, metric: &str) -> Option<u64> {
    match metric {
        "CPU Instructions" => {
            if let Some(abs) = func_config.cpu_limit {
                Some(abs)
            } else if let Some(pct) = func_config.cpu_limit_pct {
                Some(PROTOCOL_MAX_CPU_INSTRUCTIONS * pct / 100)
            } else {
                None
            }
        }
        "Read Bytes" => {
            if let Some(abs) = func_config.read_limit {
                Some(abs)
            } else if let Some(pct) = func_config.read_limit_pct {
                Some(PROTOCOL_MAX_READ_BYTES * pct / 100)
            } else {
                None
            }
        }
        "Write Bytes" => {
            if let Some(abs) = func_config.write_limit {
                Some(abs)
            } else if let Some(pct) = func_config.write_limit_pct {
                Some(PROTOCOL_MAX_WRITE_BYTES * pct / 100)
            } else {
                None
            }
        }
...
fn emit_check_failure_entries(
    ...
    for metric in ["CPU Instructions", "Read Bytes", "Write Bytes"] {
        let limit = resolve_limit(func_config, metric);
            ...
            match simulate_function(&contract_id, &source, &network, &function, &func_args)? {
                SimulationOutcome::Metrics {
                    ...
                    for (metric, value) in [
                        ...
                        let limit = func_config.and_then(|cfg| resolve_limit(cfg, metric));
